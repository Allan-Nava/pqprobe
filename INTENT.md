...
2. **Be safe to point at production.** A handshake and a close. No request, no
   ...
   `STARTTLS`, an eight-byte `SSLRequest`, MySQL's 32-byte one, an LDAP
   StartTLS request, an XMPP stream header — because ports 587, 143, 5432,
   3306, 389, 5222, 21 and 119 cannot be reached any other way; still no mail,
   no query, no bind, no credential. That is a product property, not an
   aesthetic, and CI enforces it.
