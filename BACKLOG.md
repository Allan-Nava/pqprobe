...
- [x] **PQ-70 — Say which contract a document speaks**: the JSON carries the
  ...
  Shipped as `output.Schema`, carried by `--json`, `--findings=wrapped` and a
  `pqprobe_schema_version` series in the textfile — the flat array has nowhere
  to put it, so the page says so rather than leaving a consumer to wonder.
  `--baseline` refuses a document from a **newer** schema and still reads one
  written before the field existed: every baseline on disk today has no number,
  and an upgrade that lost a history would be worse than no gate.
  `docs/schema.md` is generated by a reflection walk over the document types,
