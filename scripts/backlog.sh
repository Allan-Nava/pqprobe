...
# stable PQ-n id and a trailing `<!-- pq: ... -->` metadata comment. ROADMAP.md is
...
#     --apply | --milestones M11,M12 | --body PQ-n | --title PQ-n
#
...
lint() {
	...
	END {
		for (i = 1; i <= max; i++)
			if (!("PQ-" i in seen)) err("PQ-" i " is missing — ids run 1..N with no gaps; retire an item with status done, never by deleting it")
...
issue_body() {
	awk -v want="$1" -F'\t' \
		...
		-v repo_blob="https://github.com/Allan-Nava/pqprobe/blob/main" '
	...
	$1 == "I" && $3 == want {
		...
		print ""
		print "`BACKLOG.md` is the single source of truth: it carries the stable `PQ-n` id that commits and the CHANGELOG reference, and [ROADMAP.md](" repo_blob "/ROADMAP.md) is generated from it. This issue is a view of that item, kept in step by `scripts/backlog.sh issues --apply`, so closing it means ticking the item in the backlog and regenerating the roadmap in the same commit."
	}
...
# PQ-63 is literally named `--profile apple|dash-if|none`. Splitting on it truncated
...
# titled "PQ-n — " with nothing after the dash: `ver` is empty for an open item, so
...
issues_cmd() {
	...
	while [ $# -gt 0 ]; do
		...
		--title)
			...
			title_of="${1:-}"
			[ -n "$title_of" ] || {
				echo "backlog.sh: --title needs an PQ-n id" >&2
				exit 2
			}
			;;
		--body)
			...
			body_of="${1:-}"
			[ -n "$body_of" ] || {
				echo "backlog.sh: --body needs an PQ-n id" >&2
				exit 2
			}
			;;
		*)
...
*)
	...
	echo "       scripts/backlog.sh issues [--apply] [--milestones M11,M12] [--body PQ-n]" >&2
	exit 2
	;;
esac
