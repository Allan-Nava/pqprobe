...
# row in three the first time an item title contained a `|` (PQ-63): the
...
- [x] **PQ-1 — Shipped and high**: nothing more to do here.
  <!-- pq: prio=high size=S labels=probe ver=0.1.0 -->
- [ ] **PQ-2 — `--profile apple|dash-if|none`**: a pipe is legal in a title, and
  PQ-63 is named exactly like this.
  <!-- pq: prio=med size=M labels=cli -->

## M2 — Later things <!-- ms: target=v0.2.0 phase=later -->

- [ ] **PQ-3 — An em dash — right here**: it has to survive the round trip.
  <!-- pq: prio=low size=L labels=docs,tests -->
- [ ] **PQ-4 — Open and high, listed after a low one**: priority decides the
  order inside a milestone, not the order they were written in.
  <!-- pq: prio=high size=M labels=verdict -->
EOF
...
checks=$((checks + 1))
if [ "$(grep -c '^| \*\*PQ-2\*\*' "$tmp/ROADMAP.md" 2>/dev/null || grep -c 'PQ-2' "$tmp/ROADMAP.md")" -ge 1 ]; then
	echo "ok   the row carrying a pipe is still one row"
else
	fail "PQ-2 is missing from the roadmap entirely"
fi

# Every row of one markdown table has the same number of cells, and an escaped
# pipe is not one of them. This is the assertion PQ-63 would have failed: the
...
checks=$((checks + 1))
if [ "$(grep -n 'PQ-4' "$tmp/ROADMAP.md" | head -1 | cut -d: -f1)" \
	...
	fail "PQ-4 (high) is listed after PQ-3 (low)"
fi
...
assert_contains "next leads with the highest priority open item" "PQ-4" "$tmp/next.txt"
assert_absent "next does not offer work that is already done" "PQ-1" "$tmp/next.txt"
checks=$((checks + 1))
...
# An id used twice: two commits reference PQ-2 and mean different work.
printf '%s\n%s\n' "$head" '- [ ] **PQ-1 — One**: a.
  <!-- pq: prio=med size=S labels=cli -->
- [ ] **PQ-1 — Two**: b.
  <!-- pq: prio=med size=S labels=cli -->' >"$tmp/dup.md"
assert_fails "a duplicate id is rejected" "$tmp/dup.md" "PQ-1"
...
printf '%s\n%s\n' "$head" '- [ ] **PQ-1 — One**: a.
  <!-- pq: prio=med size=S labels=cli -->
- [ ] **PQ-3 — Three**: the gap where PQ-2 was.
  <!-- pq: prio=med size=S labels=cli -->' >"$tmp/gap.md"
assert_fails "a gap in the sequence is rejected" "$tmp/gap.md" "PQ-2"
...
printf '%s\n%s\n' "$head" '- [ ] **PQ-1 — One**: a.
  <!-- pq: prio=urgent size=S labels=cli -->' >"$tmp/prio.md"
assert_fails "an unknown priority is rejected" "$tmp/prio.md" "prio"

printf '%s\n%s\n' "$head" '- [ ] **PQ-1 — One**: a.
  <!-- pq: prio=med size=XXL labels=cli -->' >"$tmp/size.md"
...
printf '%s\n%s\n' "$head" '- [ ] **PQ-1 — One**: a.' >"$tmp/nometa.md"
assert_fails "an item with no metadata is rejected" "$tmp/nometa.md" "PQ-1"
...
printf '%s\n%s\n' "$head" '- [x] **PQ-1 — One**: shipped, apparently.
  <!-- pq: prio=med size=S labels=cli -->' >"$tmp/nover.md"
...
" '- [ ] **PQ-1 — One**: still open.
  <!-- pq: prio=med size=S labels=cli -->' >"$tmp/phase.md"
